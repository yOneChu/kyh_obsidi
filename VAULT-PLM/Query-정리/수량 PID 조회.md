...
with ouid as  
         (select A.vf$ouid AS VFOID  
          ...
          where A.vf$identity = B.id$ouid  
            ...
            AND SUBSTR(A.MD$CDATE, 0, 4) = '2025'  
          )  
SELECT  
          PE.SEQ  
        ...
        WHERE  
        --(PE.CMT LIKE '%BDBMIR%' AND PE.CMT LIKE '%HIGHGLOSSY%') AND  
           --(SELECT MD$NUMBER FROM PRODUCT$VF WHERE VF$OUID = PE.PRODUCTOUID) = 'E15677L01' AND        PE.PRODUCTOUID IN (SELECT VFOID FROM ouid)  
AND (SELECT MD$NUMBER FROM BLOCKNO$SF WHERE SF$OUID =  DECODE(NP.BLOCKNO, NULL, NULL, HEXTODEC(UPPER(SUBSTR(NP.BLOCKNO, 12))))) = 'B259C01'
```