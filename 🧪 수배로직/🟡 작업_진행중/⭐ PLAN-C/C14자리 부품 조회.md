...
with ouid as  
 ...
 SELECT  
                     ...
                     , (SELECT COD(E.EL_ASPC) FROM ELV_INFO$ID A, ELV_INFO$VF E  
                        WHERE A.ID$OUID = E.VF$IDENTITY AND E.vf$ouid = A.id$wip  
                        AND E.MD$NUMBER = (SELECT F.MD$NUMBER FROM PRODUCT$VF F WHERE F.VF$OUID = PE.PRODUCTOUID) ) AS EL_ASPC  
...
--                     AND  
--                         (SELECT EL_ASPC FROM ELV_INFO$ID A, ELV_INFO$VF E  
--                        WHERE A.ID$OUID = E.VF$IDENTITY AND E.vf$ouid = A.id$wip  
--                        AND E.MD$NUMBER = (SELECT F.MD$NUMBER FROM PRODUCT$VF F WHERE F.VF$OUID = PE.PRODUCTOUID) ) IS NOT NULL  
                    AND VP.EXCEPTFLAG IS NOT NULL  
                    --AND PE.QTY != 0  
                    --EL_ASPC:시방서  
;
```