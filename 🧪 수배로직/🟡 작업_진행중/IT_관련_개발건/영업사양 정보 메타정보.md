...
## 14. 특정 호기 영업사양 정보 추출 API  
호기번호(현장/제품 번호)를 파라미터로 전달받아 **해당 호기의 영업사양 데이터**를 즉시 추출합니다.  
  
- **URL**: `https://plmpro.hdel.co.kr/jsp/help/salesInfoFromProductViewJson.jsp?productNumber=N28431L01`  
- **HTTP Method**: `GET`  
- **파라미터**:  
  | 파라미터명 | 타입 | 필수 여부 | 설정값/예시 | 설명 |  
  |---|---|---|---|---|  
  | `productNumber` | String | 필수 | `TEST-626617` | 조회 대상 호기번호 (현장/제품 번호) |  
  
- **호출 예시**:  
  ```text  
  GET https://plmpro.hdel.co.kr/jsp/help/salesInfoFromProductViewJson.jsp?productNumber=TEST-626617  
  ```
```